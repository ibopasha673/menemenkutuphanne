...
export default function Home() {
  ...
  // =========================================================
  // DUYURU OTOMATİK GEÇİŞ (10 Saniye)
  // =========================================================
  useEffect(() => {
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col bg-zinc-950 text-white overflow-x-hidden">
      ...
      <header className="absolute top-0 left-0 w-full z-30 flex flex-col md:flex-row justify-between items-center px-4 md:px-8 py-4 md:py-5 bg-gradient-to-b from-black/80 via-black/40 to-transparent gap-4">
        {/* LOGO VE SOL ÜST DUYURU */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-6 w-full md:w-auto justify-between md:justify-start">
          ...
          {/* SOL ÜST KÖŞE (PC): LOGONUN YANI / MOBİLDE ÜST ÜSTE BİNMEYEN DUYURU KUTUCUĞU */}
          {duyurular.length > 0 && (
            <div 
              onClick={() => setIsDuyuruModalOpen(true)}
              className="max-w-[240px] md:max-w-xs bg-black/70 hover:bg-black/90 backdrop-blur-md px-3 py-2 md:py-2.5 rounded-xl border border-emerald-500/40 shadow-xl cursor-pointer transition-all group mt-1 sm:mt-0"
            ...
            >
              <div className="flex items-center gap-1.5 text-emerald-400 font-bold text-[11px] mb-0.5">
                <Bell className="w-3 h-3 animate-bounce" /> Güncel Duyuru (Tıkla)
              </div>
              <h4 className="text-[11px] font-semibold text-white line-clamp-1 group-hover:text-emerald-300 transition">
